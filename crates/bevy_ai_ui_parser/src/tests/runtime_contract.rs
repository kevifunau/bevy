...
use crate::core::model::{bui_node, BuiStyles, BuiVisuals};
...
#[test]
fn runtime_build_node_maps_box_sizing() {
    let styles = BuiStyles {
        box_sizing: Some("content-box".to_string()),
        ..Default::default()
    };
    let node = build_node(&styles, &BuiVisuals::default()).expect("node should build");

    assert_eq!(node.box_sizing, BoxSizing::ContentBox);
}

#[test]
fn runtime_build_node_maps_two_axis_overflow() {
    let styles = BuiStyles {
        overflow: Some("hidden scroll".to_string()),
        ..Default::default()
    };
    let node = build_node(&styles, &BuiVisuals::default()).expect("node should build");
...
#[test]
fn runtime_stage_fit_modes_compute_expected_scale() {
    let fit = BuiStageFit {
        design_width: 400.0,
        design_height: 200.0,
        mode: BuiStageFitMode::ScaleDown,
    };
    assert_eq!(stage_fit_scale(fit, Vec2::new(800.0, 800.0)), Vec2::ONE);

    let contain = BuiStageFit {
        mode: BuiStageFitMode::Contain,
        ..fit
    };
    assert_eq!(
        stage_fit_scale(contain, Vec2::new(800.0, 300.0)),
        Vec2::splat(1.5)
    );

    let cover = BuiStageFit {
        mode: BuiStageFitMode::Cover,
        ..fit
    };
    assert_eq!(
        stage_fit_scale(cover, Vec2::new(800.0, 300.0)),
        Vec2::splat(2.0)
    );

    let fill = BuiStageFit {
        mode: BuiStageFitMode::Fill,
        ..fit
    };
    assert_eq!(
        stage_fit_scale(fill, Vec2::new(800.0, 300.0)),
        Vec2::new(2.0, 1.5)
    );
}
