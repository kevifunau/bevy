...
use crate::core::opendesign::{
    html::opendesign_html_to_bui_document_with_manifest, manifest::apply_manifest_to_document,
};
use crate::{opendesign_html_to_bui_json_str_with_manifest, validate_bui_json_str};
...
fn opendesign_manifest_injects_images_slicer_atlas_and_state_textures() {
    ...
    assert_eq!(hover_image.texture_path, "./assets/buttons/close_hover.png");
...
fn opendesign_manifest_json_entrypoint_validates() {
    let json = opendesign_html_to_bui_json_str_with_manifest(
        BEVY_UI_ASSET_FLOW_HTML,
        BEVY_UI_ASSET_FLOW_MANIFEST,
    )
