...
impl OpenDesignStylesheet {
    ...
    pub(crate) fn matching_state_declarations(
        ...
        let mut rules = self
            ...
            .filter_map(|rule| {
                rule.selector
                    .state_name()
                    .filter(|_| rule.selector.matches_state_template(dom_node))
    ...
    pub(crate) fn tag_declarations(&self, tag: &str) -> Vec<&(String, String)> {
        let mut rules = self
            .rules
            .iter()
            .filter(|rule| {
                rule.selector.state_name().is_none()
                    && rule.selector.pseudo_element_name().is_none()
                    && rule.selector.is_single_tag_selector(tag)
            })
            .collect::<Vec<_>>();
        rules.sort_by_key(|rule| (rule.selector.weight(), rule.order));
        rules
