...
use crate::core::interaction::components::{
    BuiBaseVisualState, BuiDisabled, BuiVisualState, BuiVisualStateDefinitions,
};
...
pub(crate) fn apply_bui_visual_states_system(
    ...
    for (entity, definitions, base_visual, explicit_state, interaction, checked, disabled) in
        &visual_states
    {
        let inherited_state =
        ...
        let state_visual =
            resolve_visual_state_name(definitions, base_state.as_deref(), auto_state)
                .and_then(|state_name| definitions.states.get(&state_name))
                .or_else(|| base_visual.map(|base| &base.0));

        let Some(state_visual) = state_visual else {
            continue;
        };
