...
pub enum BuiActionTrigger {
    ...
    HoverExit,
    /// Trigger when an editable value changes.
    ValueChanged,
    /// Trigger when the focused control submits, for example by pressing Enter.
    Submit,
    /// Trigger when a control receives focus.
    Focus,
    /// Trigger when a control loses focus.
    Blur,
    /// Trigger when a scrollable control's scroll position changes.
    Scroll,
    /// Trigger when a selection control chooses a new item.
    SelectionChanged,
}
