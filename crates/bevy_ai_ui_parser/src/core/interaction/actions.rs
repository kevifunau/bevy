...
fn parse_action_trigger(value: &str) -> Result<BuiActionTrigger, String> {
    match normalize_token(value).as_str() {
        ...
        "value_changed" | "change" | "changed" => Ok(BuiActionTrigger::ValueChanged),
        "submit" | "submitted" => Ok(BuiActionTrigger::Submit),
        "focus" | "focused" => Ok(BuiActionTrigger::Focus),
        "blur" | "unfocused" => Ok(BuiActionTrigger::Blur),
        "scroll" | "scrolled" => Ok(BuiActionTrigger::Scroll),
        "selection_changed" | "selection" | "select" | "selected" => {
            Ok(BuiActionTrigger::SelectionChanged)
        }
        _ => Err(format!(
    ...
    for (entity, interaction, id, actions, disabled) in &interactions {
        ...
        for action in &actions.0 {
            ...
            let matched = match trigger {
                ...
                | BuiActionTrigger::Scroll
                | BuiActionTrigger::SelectionChanged => false,
            };
