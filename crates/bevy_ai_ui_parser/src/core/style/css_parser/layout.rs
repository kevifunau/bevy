...
pub(crate) fn parse_box_sizing(value: &str) -> Result<BoxSizing, String> {
    match normalize_token(value).as_str() {
        "border_box" | "border-box" => Ok(BoxSizing::BorderBox),
        "content_box" | "content-box" => Ok(BoxSizing::ContentBox),
        _ => Err(format!("Invalid box_sizing '{value}'.")),
    }
}
...
pub(crate) fn parse_overflow(value: &str) -> Result<Overflow, String> {
    ...
    match normalized.as_str() {
        ...
        _ => {
            ...
            if parts.len() == 2 {
                return Ok(Overflow {
                    x: parse_overflow_axis(parts[0])?,
                    y: parse_overflow_axis(parts[1])?,
                });
            }
            Err(format!("Invalid overflow '{value}'."))
        }
    }
}

fn parse_overflow_axis(value: &str) -> Result<OverflowAxis, String> {
    match value {
        "visible" => Ok(OverflowAxis::Visible),
        "clip" => Ok(OverflowAxis::Clip),
        "hidden" => Ok(OverflowAxis::Hidden),
        "scroll" => Ok(OverflowAxis::Scroll),
        _ => Err(format!("Invalid overflow axis '{value}'.")),
    }
}
