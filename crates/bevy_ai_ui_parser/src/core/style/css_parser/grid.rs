...
pub(crate) fn parse_grid_auto_tracks(value: &str) -> Result<Vec<GridTrack>, String> {
    let declarations = split_grid_track_declarations(value)?;

    declarations
        .into_iter()
        .map(|declaration| parse_single_grid_auto_track(&declaration))
        .collect()
}
...
fn parse_single_grid_track(value: &str) -> Result<RepeatedGridTrack, String> {
    ...
    if let Some(args) = normalized
        .strip_prefix("fit_content_px(")
        .and_then(|value| value.strip_suffix(')'))
    {
        if let Some((repetition, limit)) = args.split_once(',') {
            let repetition = repetition
                .parse::<u16>()
                .map_err(|error| format!("Invalid grid track repetition in '{value}': {error}"))?;
            let limit = parse_number(limit)?;
            return Ok(RepeatedGridTrack::fit_content_px(repetition, limit));
        }
        return parse_number(args).map(|limit| RepeatedGridTrack::fit_content_px(1, limit));
    }

    if let Some(args) = normalized
        .strip_prefix("fit_content_percent(")
        .and_then(|value| value.strip_suffix(')'))
    {
        if let Some((repetition, limit)) = args.split_once(',') {
            let repetition = repetition
                ...
                .map_err(|error| format!("Invalid grid track repetition in '{value}': {error}"))?;
            let limit = parse_number(limit)?;
            return Ok(RepeatedGridTrack::fit_content_percent(repetition, limit));
        }
        return parse_number(args).map(|limit| RepeatedGridTrack::fit_content_percent(1, limit));
    }

    Err(format!("Invalid grid track declaration '{value}'."))
}

fn parse_single_grid_auto_track(value: &str) -> Result<GridTrack, String> {
    let normalized = value.trim().replace(' ', "");
    ...
    if normalized.eq_ignore_ascii_case("min_content") {
        return Ok(GridTrack::min_content());
    }

    if normalized.eq_ignore_ascii_case("max_content") {
        return Ok(GridTrack::max_content());
    }
    ...
    if let Some(argument) = normalized
        ...
        .and_then(|value| value.strip_suffix(')'))
    {
        return parse_number(argument).map(GridTrack::px);
    }
    ...
    if let Some(argument) = normalized
        ...
        .and_then(|value| value.strip_suffix(')'))
    {
        return parse_number(argument).map(GridTrack::fr);
    }
    ...
    if let Some(argument) = normalized
        ...
        .and_then(|value| value.strip_suffix(')'))
    {
        return parse_number(argument).map(GridTrack::flex);
    }
    ...
    if let Some(argument) = normalized
        ...
        .and_then(|value| value.strip_suffix(')'))
    {
        return parse_number(argument).map(GridTrack::fit_content_px);
    }
    ...
    if let Some(argument) = normalized
        ...
        .and_then(|value| value.strip_suffix(')'))
    {
        return parse_number(argument).map(GridTrack::fit_content_percent);
    }

    Err(format!("Invalid grid auto track declaration '{value}'."))
}
...
pub(crate) fn parse_grid_placement(value: &str) -> Result<GridPlacement, String> {
        ...
        .and_then(|value| value.strip_suffix(')'))
    {
        let start = start
            ...
            .map_err(|error| format!("Invalid grid placement '{value}': {error}"))?;
        if start == 0 {
            return Err(format!("Invalid grid placement '{value}'."));
        }
        return Ok(GridPlacement::start(start));
    }
