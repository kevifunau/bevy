use crate::core::{
    model::{BuiImageConfig, BuiNode, BuiStateVisual, BuiStyles, BuiVisuals},
    style::{
        css_effects::{
        ...
        css_values::{css_background_image_url, css_color},
    },
};
        ...
        match name {
            "background" | "background-color" => {
                ...
                if let Some(texture_path) = css_background_image_url(&value) {
                    state_visual.image = Some(background_image_config(texture_path));
                }
            }
            "background-image" => {
                if let Some(texture_path) = css_background_image_url(&value) {
            ...
            "background-size" => {
                if let Some(image) = &mut state_visual.image {
                    apply_background_size_image_mode(image, &value);
                    image.background_size = Some(value.clone());
                }
            }
            "background-position" => {
                if let Some(image) = &mut state_visual.image {
                    image.background_position = Some(value.clone());
                }
            }
            "background-repeat" => {
                if let Some(image) = &mut state_visual.image {
                    image.background_repeat = Some(background_repeat_fallback(&value));
                }
            }
            "border" => {
                if let Some(color) = css_color(&value) {
...
fn background_image_config(texture_path: String) -> BuiImageConfig {
    BuiImageConfig {
        texture_path,
...
fn background_repeat_fallback(value: &str) -> String {
    if value.trim().eq_ignore_ascii_case("repeat") {
        "no-repeat".to_string()
    } else {
        value.to_string()
    }
