...
use super::{
    ...
    semantics::{
        validate_actions, validate_bindings, validate_dropdown_semantics, validate_list_semantics,
        validate_progress_semantics, validate_scroll_view_semantics, validate_slider_semantics,
...
pub(super) fn validate_bui_node(
    ...
    validate_slider_semantics(node).map_err(|error| format!("{path}: {error}"))?;
    validate_scroll_view_semantics(node).map_err(|error| format!("{path}: {error}"))?;
    validate_dropdown_semantics(node).map_err(|error| format!("{path}: {error}"))?;
    ...
    match node.node_type() {
        ...
        BuiNodeType::Slider => {
            if node.semantics.slider.is_none() {
                return Err(format!("{path}: Slider requires slider semantics."));
            }
            reject_config(node.content.text.is_some(), path, "text_config")?;
            if let Some(image_config) = &node.content.image {
                validate_image_config(image_config).map_err(|error| format!("{path}: {error}"))?;
