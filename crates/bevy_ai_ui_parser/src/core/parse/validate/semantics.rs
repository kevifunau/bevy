...
pub(super) fn validate_state_visuals(
    states: &BTreeMap<String, BuiStateVisual>,
) -> Result<(), String> {
...
pub(super) fn validate_slider_semantics(node: &BuiNode) -> Result<(), String> {
    let Some(slider) = &node.semantics.slider else {
        return Ok(());
    };
    if slider.max < slider.min {
        return Err("slider.max must be greater than or equal to slider.min.".to_string());
    }
    if slider.value < slider.min || slider.value > slider.max {
        return Err("slider.value must be within slider.min..=slider.max.".to_string());
    }
    if let Some(step) = slider.step
        && step <= 0.0
    {
        return Err("slider.step must be positive when present.".to_string());
    }
    Ok(())
}

pub(super) fn validate_scroll_view_semantics(node: &BuiNode) -> Result<(), String> {
    let Some(scroll_view) = &node.semantics.scroll_view else {
        return Ok(());
    };
    if let Some(source) = &scroll_view.binding_source
        && source.trim().is_empty()
    {
        return Err("scroll_view.binding_source must not be empty when present.".to_string());
    }
    if let Some(axis) = &scroll_view.axis
        && !matches!(axis.as_str(), "x" | "y" | "both")
    {
        return Err("scroll_view.axis must be one of x, y, or both.".to_string());
    }
    Ok(())
}

pub(super) fn validate_dropdown_semantics(node: &BuiNode) -> Result<(), String> {
    if let Some(group) = &node.semantics.dropdown_group_name
        && group.trim().is_empty()
    {
        return Err("dropdown_group_name must not be empty when present.".to_string());
    }
    if let Some(source) = &node.semantics.dropdown_binding_source {
        if source.trim().is_empty() {
            return Err("dropdown_binding_source must not be empty when present.".to_string());
        }
        if node.semantics.dropdown_group_name.is_none() {
            return Err("dropdown_binding_source requires dropdown_group_name.".to_string());
        }
    }
    if let Some(value) = &node.semantics.dropdown_value {
        if value.trim().is_empty() {
            return Err("dropdown_value must not be empty when present.".to_string());
        }
        if node.semantics.dropdown_group_name.is_none() {
            return Err("dropdown_value requires dropdown_group_name.".to_string());
        }
    }
    Ok(())
}
...
fn parse_action_trigger(value: &str) -> Result<(), String> {
    match value.trim().to_ascii_lowercase().replace('-', "_").as_str() {
        "press" | "pressed" | "hover_enter" | "hovered" | "hover_exit" | "unhovered"
        | "value_changed" | "change" | "changed" | "submit" | "submitted" | "focus"
        | "focused" | "blur" | "unfocused" | "scroll" | "scrolled"
        | "selection_changed" | "selection" | "select" | "selected" => Ok(()),
        _ => Err(format!(
            "Unsupported action event '{}'. Expected one of: press, hover_enter, hover_exit, value_changed, submit, focus, blur, scroll, selection_changed.",
            value
        )),
    }
}
