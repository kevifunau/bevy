use crate::core::{
    ...
    style::css_parser::{
        ...
        parse_box_sizing, parse_color, parse_display, parse_flex_direction, parse_flex_wrap,
        parse_grid_auto_tracks, parse_grid_placement, parse_grid_tracks, parse_integer,
        parse_justify_content, parse_justify_items, parse_justify_self, parse_linebreak,
        parse_node_image_mode, parse_number, parse_overflow, parse_overflow_clip_margin,
        parse_position_type, parse_tab_group, parse_text_line_height, parse_ui_rect, parse_val,
    },
};
...
pub(super) fn validate_styles(styles: &BuiStyles) -> Result<(), String> {
    ...
    if let Some(value) = &styles.box_sizing {
    ...
    if let Some(value) = &styles.grid_auto_columns {
        parse_grid_auto_tracks(value)?;
    }
    if let Some(value) = &styles.grid_auto_rows {
        parse_grid_auto_tracks(value)?;
    }
...
pub(super) fn validate_image_config(image_config: &BuiImageConfig) -> Result<(), String> {
    ...
    if let Some(repeat) = &image_config.background_repeat
        && !matches!(
            repeat.as_str(),
            "no-repeat" | "repeat" | "repeat-x" | "repeat-y"
        )
    {
        return Err("image_config.background_repeat has unsupported value.".to_string());
    }
