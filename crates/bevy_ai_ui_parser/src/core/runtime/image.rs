...
pub(crate) fn build_image_node(
    ...
    let image_mode_value = background_image_mode_override(image_config)
        .or(image_config.image_mode.as_deref())
        .unwrap_or("auto");
    let image_mode = parse_node_image_mode(image_mode_value, image_config.slicer.as_ref())?;
...
pub(crate) fn background_image_layout(
    ...
    if image_config.background_size.is_none()
        && image_config.background_position.is_none()
        && image_config.background_repeat.is_none()
    {
        return None;
    }
    ...
    Some(BuiBackgroundImageLayout {
        ...
        repeat: image_config.background_repeat.clone(),
    })
}
...
fn compute_background_rect(
    ...
    let _repeat = layout.repeat.as_deref().unwrap_or("no-repeat");

    if background_size_preserves_full_source(size) {
        return None;
    }
...
fn background_size_is_full_stretch(size: &str) -> bool {
    ...
    matches!(
        parts.as_slice(),
        [single] if single.eq_ignore_ascii_case("100%")
    ) || matches!(
        parts.as_slice(),
        [x, y] if x.eq_ignore_ascii_case("100%") && y.eq_ignore_ascii_case("100%")
    )
}
...
mod tests {
    ...
    fn layout(size: &str) -> BuiBackgroundImageLayout {
        BuiBackgroundImageLayout {
            size: Some(size.to_string()),
            position: Some("center".to_string()),
            repeat: Some("no-repeat".to_string()),
        }
    }

    #[test]
    fn contain_background_size_does_not_crop_source_image() {
        let rect = compute_background_rect(
            Vec2::new(420.0, 204.0),
    ...
    #[test]
    fn full_stretch_background_size_does_not_crop_source_image() {
        let rect = compute_background_rect(
            Vec2::new(684.0, 148.0),
            Vec2::new(342.0, 74.0),
            &layout("100% 100%"),
        );
        assert_eq!(rect, None);
    ...
    #[test]
    fn cover_background_size_crops_source_image_to_node_aspect() {
        let rect = compute_background_rect(
            Vec2::new(400.0, 200.0),
            Vec2::new(100.0, 100.0),
            &layout("cover"),
        )
        .expect("cover should crop the source image");

        assert_eq!(rect.min, Vec2::new(100.0, 0.0));
        assert_eq!(rect.max, Vec2::new(300.0, 200.0));
    }
}
