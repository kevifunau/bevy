...
use crate::core::{
    ...
    style::css_parser::{
        ...
        parse_box_sizing, parse_display, parse_flex_direction, parse_flex_wrap,
        parse_grid_auto_tracks, parse_grid_placement, parse_grid_tracks, parse_integer,
        parse_justify_content, parse_justify_items, parse_justify_self, parse_number,
        parse_overflow, parse_overflow_clip_margin, parse_position_type, parse_tab_group,
        parse_ui_rect, parse_val, parse_visibility,
    },
};
...
pub(crate) fn insert_style_components(
    ...
    if node.layout.styles.fixed_node.unwrap_or(false) {
        // FixedNode component is only available in Bevy 0.20+; skip on 0.19.
    }
...
pub(crate) fn build_node(styles: &BuiStyles, visuals: &BuiVisuals) -> Result<Node, String> {
    ...
    if let Some(value) = &styles.box_sizing {
        node.box_sizing = parse_box_sizing(value)?;
    }
    ...
    if let Some(value) = &styles.grid_auto_columns {
        node.grid_auto_columns = parse_grid_auto_tracks(value)?;
    }
    if let Some(value) = &styles.grid_auto_rows {
        node.grid_auto_rows = parse_grid_auto_tracks(value)?;
    }
